...
const cardData: BentoCardProps[] = [
  ...
  },
  {
    color: '#00A74F',
    label: 'О нас',
    // --- УПРОЩЕННЫЙ БЛОК ОПИСАНИЯ ---
    description: (
      <div>
        <h4>О компании «АСР Лизинг»</h4>
        <p>ООО «АСР Лизинг» — одна из крупнейших лизинговых компаний Таджикистана, основанная в 2013 году Исламской Корпорацией по Развитию Частного Сектора (ИКРЧС) и группой инвесторов. Сегодня акционерами являются ИКРЧС и Евразия Груп АГ.
Мы специализируемся на финансовом лизинге (иджара) техники и оборудования, предоставляя альтернативные источники финансирования для бизнеса.</p>
        <h4>Наша цель </h4>
        <p>поддержка малого и среднего бизнеса (МСП) и содействие экономическому росту Таджикистана. Мы понимаем, что МСП — движущая сила экономики, способствующая повышению уровня жизни населения, росту инвестиций и занятости.
Мы стремимся предоставлять быстрые и надежные лизинговые решения, учитывая потребности наших клиентов и условия рынка.
</p>
      </div>
    ),
  },
  {
    label: 'Полиграфическое оборудование',
    imageUrl: photo2,
  },
  {
    label: 'Производственное оборудование',
    imageUrl: photo3,
  }
];
...
const GlobalSpotlight: React.FC<{
  gridRef: React.RefObject<HTMLDivElement | null>; disableAnimations?: boolean;
  enabled?: boolean; spotlightRadius?: number; glowColor?: string;
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          ...
          const isAboutCard = card.label === 'О нас';

          const baseClassName = [
            'magic-bento-card',
            (textAutoHide && !isAboutCard && !isImageCard) ? 'magic-bento-card--text-autohide' : '',
            enableBorderGlow ? 'magic-bento-card--border-glow' : '',
            isAboutCard ? 'magic-bento-card--about-us' : '',
            isImageCard ? 'magic-bento-card--image-bg' : ''
          ].filter(Boolean).join(' ');
          ...
          if (isImageCard) {
            cardStyle.backgroundImage = `url(${card.imageUrl})`;
          } else {
            cardStyle.backgroundColor = card.color;
          }
          ...
          return enableStars ? (
            <ParticleCard key={index} {...cardProps} disableAnimations={shouldDisableAnimations} particleCount={particleCount}
              glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
              {cardContent}
            </ParticleCard>
          ) : (
            <div key={index} {...cardProps}>{cardContent}</div>
          );
        })}
      </BentoCardGrid>
    </>
